...
generateToekn : protectedProcedure.mutation(async ({ctx}) => {
    ...
    const experienceTime = Math.floor(Date.now()/100) + 3600;
    const isUsedAt = Math.floor(Date.now()/100)-60;
    const token = streamVideoClient.generateUserToken({user_id:ctx.auth.user.id,experienceTime,
      valadity_in_seconds : isUsedAt,
...
,
  update:protectedProcedure
    ...
    .mutation(async ({ input, ctx }) => {
     const [updatedMeeting] = await db.update(meetings).set(input).where(and(eq(meetings.id, input.id), eq(meetings.UserId, ctx.auth.user.id))
    ).returning();
    ;
   if(!updatedMeeting){
  ...
  remove:protectedProcedure
    ...
    .mutation(async ({ input, ctx }) => {
     const [removedMeeting] = await db.delete(meetings).where(and(eq(meetings.id, input.id), eq(meetings.UserId, ctx.auth.user.id))
    ).returning();
    ;
   if(!removedMeeting){
    throw new TRPCError({ code: "NOT_FOUND", message: "Meeting not found" });
  ...
  getMany: protectedProcedure
    .input(
      z.object({
        ...
        status:z.enum(["Upcoming","active","cancelled","processing","completed"]).nullish(),
      })
    )
